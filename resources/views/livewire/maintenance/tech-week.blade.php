            ...
            <thead>
                <tr>
                    @for($i = 0; $i < 7; $i++)
                        ...
                        <th class="px-2 py-1 text-center">{{ $d->format('D d-m') }}</th>
                    @endfor
                </tr>
            </thead>
            <tbody>
                <tr>
                    @for($i = 0; $i < 7; $i++)
                        @php $date = $start->copy()->addDays($i)->format('Y-m-d'); @endphp
                        <td class="border px-2 py-2 align-top">
                            @forelse($appointments[$date] ?? [] as $a)
                                <a href="{{ route('maintenance.view', $a->id) }}"
                                   class="block mb-2 p-2 border rounded hover:bg-gray-50">
                            ...
                            @empty
                                <div class="text-xs text-neutral-400">Geen</div>
                            @endforelse
                        </td>
                    @endfor
                </tr>
